...
class MediaPipeFatigueTracker:
    ...
    def calibrate_blink_threshold(self):
        ...
        # just means that if no EAR values were collected, it indicates a calibration failure.
    ...
    def select_camera():
        """Prompt user to select a camera index if multiple are available, with friendly names."""
        ...
        if len(available_cams) == 0:
            ...
            idx = available_cams[0]
            print(f"Only one camera found. Using camera index {idx} ({cam_names.get(idx, 'Unknown')}).")
            return idx
        else:
            while True:
                try:
                    ...
                    user_input = input(f"Enter camera index to use: ")
                    idx = int(user_input)
