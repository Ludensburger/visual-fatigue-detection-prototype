...
class VisualFatigueTracker:
    def __init__(self, output_dir="fatigue_data", show_video=True):
        ...
        try:
            ...
            print("MediaPipe not installed.  Dlib will be used.")
            self.mp_face_mesh = None
            self.face_mesh = None
    ...
    def process_frame(self, frame):
        ...
        # filepath: c:\Users\Bernard\Desktop\ryu-files\code\python\cs-elec2\visual-fatigue-detection\vfd-g.py
        ...
        if self.face_mesh:
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    # --- Example: Draw ALL landmarks ---
                    if self.show_video:
                        for landmark in face_landmarks.landmark:
                            x, y = int(landmark.x * img_width), int(landmark.y * img_height)
                            cv2.circle(frame, (x, y), 1, (0, 255, 0), -1) # Green for MediaPipe

                    # --- Example:  Calculate MAR using MediaPipe landmarks (approximate) ---
                    # Note:  MediaPipe landmark indices are different from dlib.  These are just examples.
                    try:
                        ...
                        if self.show_video:
                            cv2.putText(frame, f"MAR (MediaPipe): {mar:.2f}", (10, 270), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2) # Blue
